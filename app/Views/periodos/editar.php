...
<div class="max-w-2xl">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Editar periodo académico</h1>
            <p class="text-gray-500 text-sm">Asigna las fechas del periodo y si es el actual</p>
        </div>
        <a href="<?= url('periodos') ?>" class="text-[#005880] text-sm font-medium hover:underline">← Volver</a>
    </div>

    <form method="post" action="<?= url('periodos/actualizar') ?>" class="bg-white rounded-xl shadow p-6 space-y-4">
        <input type="hidden" name="_csrf" value="<?= e(Request::csrfToken()) ?>">
        <input type="hidden" name="id" value="<?= (int) $periodo['id'] ?>">

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Nombre del periodo *</label>
            <input type="text" name="nombre" required value="<?= e($periodo['nombre']) ?>"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="fecha_inicio" required value="<?= e($periodo['fecha_inicio']) ?>"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0b6f9e] outline-none">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Fecha de fin *</label>
                <input type="date" name="fecha_fin" required value="<?= e($periodo['fecha_fin']) ?>"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0b6f9e] outline-none">
            </div>
        </div>

        <div class="flex items-center gap-2">
            <input type="checkbox" name="activo" value="1" id="activo" <?= $periodo['activo'] ? 'checked' : '' ?>
